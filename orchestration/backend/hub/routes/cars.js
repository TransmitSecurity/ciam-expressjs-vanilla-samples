...
carsRouter.get(['/', '/home'], async function (req, res) {
  ...
  res.redirect(`/pages/11-sso-demo/cars/home.html?${params.toString()}`);
});
...
carsRouter.get('/complete', async function (req, res) {
  // TODO add error handling, omitted for sample clarity
  const params = new URLSearchParams(req.query);
  res.redirect(`/pages/11-sso-demo/cars/complete.html?${params.toString()}`);
});

// Render login page
carsRouter.get('/login', async function (req, res) {
  // TODO add error handling, omitted for sample clarity
  res.redirect('/pages/11-sso-demo/cars/login.html');
});
...
carsRouter.post('/fetch-tokens', common.utils.rateLimiter(), async function (req, res) {
  ...
  const tokens = await common.tokens.getUserTokens(
    req.body.authCode,
    process.env.VITE_TS_CLIENT_ID_ORC_SSOCARS,
    process.env.TS_CLIENT_SECRET_ORC_SSOCARS,
    process.env.VITE_TS_REDIRECT_URI_ORC_SSOCARS,
  );
