...
airRouter.get(['/', '/home'], async function (req, res) {
  ...
  res.redirect(`/pages/11-sso-demo/air/home.html?${params.toString()}`);
});
...
airRouter.get('/complete', async function (req, res) {
  // TODO add error handling, omitted for sample clarity
  const params = new URLSearchParams(req.query);
  res.redirect(`/pages/11-sso-demo/air/complete.html?${params.toString()}`);
});

// Render login page
airRouter.get('/login', async function (req, res) {
  // TODO add error handling, omitted for sample clarity
  res.redirect('/pages/11-sso-demo/air/login.html');
});
...
airRouter.post('/fetch-tokens', common.utils.rateLimiter(), async function (req, res) {
  ...
  const tokens = await common.tokens.getUserTokens(
    req.body.authCode,
    process.env.VITE_TS_CLIENT_ID_ORC_SSOAIR,
    process.env.TS_CLIENT_SECRET_ORC_SSOAIR,
    process.env.VITE_TS_REDIRECT_URI_ORC_SSOAIR,
  );
