...
hubRouter.get('/complete', async function (req, res) {
  ...
  res.redirect(`/pages/11-sso-demo/hub/complete.html?${params.toString()}`);
});

// Render login page
hubRouter.get(['/', '/login'], async function (req, res) {
  const params = new URLSearchParams(req.query);
  res.redirect(`/pages/11-sso-demo/hub/login.html?${params.toString()}`);
});

// Render login page
hubRouter.get(['/ido-idp'], async function (req, res) {
  const params = new URLSearchParams(req.query);
  res.redirect(`/pages/11-sso-demo/hub/ido-idp.html?${params.toString()}`);
});
...
hubRouter.get('/signup', async function (req, res) {
  res.redirect('/pages/11-sso-demo/hub/signup.html');
});
