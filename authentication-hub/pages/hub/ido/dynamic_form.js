...
<button id="delete_button" class="full-width danger hidden"
  ...
  <div
	id="dynamic_form_error"
	style="font-family: Sans-serif; color: #0d182d; font-size: 12px;"
	class="column"
  >Notice: This form was auto generated by Dynamic Form plugin, and can only be used for non-production purposes.
  </div>
</div>`;
    ...
    responseOptions.forEach((option /*, index*/) => {
      if (option.type == 'custom') {
        if (option.label && option.id) {
          let elem = null;
          if (String(option.id).endsWith('/link')) {
            elem = createEscapeOptionLink(option);
          } else {
            elem = createEscapeOptionButton(option);
          }
...
function createEscapeOptionLink(option) {
