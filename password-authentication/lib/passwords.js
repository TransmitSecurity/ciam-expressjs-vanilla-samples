 ...
 */
async function resetByOldPassword(
  username,
  oldPassword,
  newPassword,
  clientToken
  ...
  const resetTokenResult = await getResetTokenByOldPassword(
    username,
    oldPassword,
    clientToken
  );
  ...
  return await resetPassword(
    resetTokenResult.response.result,
    newPassword,
    clientToken
  );
}
...
async function getResetTokenByOldPassword(username, password, clientToken) {
  ...
  const response = await fetch(
    `${userUrl}/auth/password/reset/password/validate`,
    {
      method: 'post',
      headers: {
        'Content-Type': 'application/json',
